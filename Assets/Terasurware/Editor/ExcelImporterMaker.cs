		...
		foreach (Object obj in Selection.objects) {
			...
			using (FileStream stream = File.Open( window.filePath, FileMode.Open, FileAccess.Read, FileShare.ReadWrite )) {
				...
				for (int i = 0; i < titleRow.LastCellNum; i++) {
					...
					if (parser.isArray) parser.name = parser.name.Remove( parser.name.LastIndexOf( "[]" ) );
					...
					if (window.typeList.Count > 0) {
						...
						int lastParserIndex = window.typeList.Count - 1;
						while (lastParserIndex >= 0) {
							lastParser = window.typeList[lastParserIndex];
		...
		foreach (ExcelRowParameter row in typeList) {
			if (row.isEnable) {
				if (!row.isArray) {
					...
					switch (row.type) {
						case ValueType.BOOL:
							builder.AppendFormat( tab + "cell = row.GetCell({1}); p.{0} = (cell == null ? false : cell.BooleanCellValue);", row.name, row.rowCount );
							break;
						case ValueType.DOUBLE:
							builder.AppendFormat( tab + "cell = row.GetCell({1}); p.{0} = (cell == null ? 0.0 : cell.NumericCellValue);", row.name, row.rowCount );
							break;
						case ValueType.INT:
							builder.AppendFormat( tab + "cell = row.GetCell({1}); p.{0} = (int)(cell == null ? 0 : cell.NumericCellValue);", row.name, row.rowCount );
							break;
						case ValueType.FLOAT:
							builder.AppendFormat( tab + "cell = row.GetCell({1}); p.{0} = (float)(cell == null ? 0 : cell.NumericCellValue);", row.name, row.rowCount );
							break;
						case ValueType.STRING:
							builder.AppendFormat( tab + "cell = row.GetCell({1}); p.{0} = (cell == null ? \"\" : cell.StringCellValue);", row.name, row.rowCount );
							break;
					}
				} else {
					...
					if (!isInbetweenArray) {
						...
						for (int i = 0; i < arrayLength; ++i) {
							...
							switch (row.type) {
								case ValueType.BOOL:
									builder.AppendFormat( tab + "cell = row.GetCell({1}); p.{0}[{2}] = (cell == null ? false : cell.BooleanCellValue);", row.name, rowCountList[i], i );
									break;
								case ValueType.DOUBLE:
									builder.AppendFormat( tab + "cell = row.GetCell({1}); p.{0}[{2}] = (cell == null ? 0.0 : cell.NumericCellValue);", row.name, rowCountList[i], i );
									break;
								case ValueType.INT:
									builder.AppendFormat( tab + "cell = row.GetCell({1}); p.{0}[{2}] = (int)(cell == null ? 0 : cell.NumericCellValue);", row.name, rowCountList[i], i );
									break;
								case ValueType.FLOAT:
									builder.AppendFormat( tab + "cell = row.GetCell({1}); p.{0}[{2}] = (float)(cell == null ? 0.0 : cell.NumericCellValue);", row.name, rowCountList[i], i );
									break;
								case ValueType.STRING:
									builder.AppendFormat( tab + "cell = row.GetCell({1}); p.{0}[{2}] = (cell == null ? \"\" : cell.StringCellValue);", row.name, rowCountList[i], i );
									break;
							}
						}
					}
